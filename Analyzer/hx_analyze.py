# -*- coding: utf-8 -*-
"""
Created on Sun Jan  9 00:26:41 2022
...
from Analyzer.hxfactor_analyzer import hx_performance as pef, hx_plotting as pl
from Analyzer.hxfactor_analyzer.hx_prepare import (get_clean_factor_and_forward_returns, rate_of_return,
                      std_conversion)
from Analyzer.hxfactor_analyzer.hx_plot_utils import _use_chinese
from Analyzer.hxfactor_analyzer.hx_utils import convert_to_forward_returns_columns, ensure_tuple
...
class FactorAnalyzer(object):
    ...
    ----------
        factor : 因子值
            ...
            1. pd.DataFrame: index 为日期, columns 为资产, values 为因子的值
            2. pd.Series: index 为日期和资产的 MultiIndex,values 为因子的值
        prices : 用于计算因子远期收益的价格数据
            # pd.DataFrame / function
            1. pd.DataFrame: index 为日期, columns 为资产价格数据必须覆盖因子分析时间段
            以及额外远期收益计算中的最大预期期数.
            2. function: 输入参数为 securities, start_date, end_date, count, 返回值为
            价格数据的 DataFrame
        groupby : 分组数据, 默认为 None
            # pd.DataFrame / dict / function
            1. pd.DataFrame: index 为日期, columns 为资产，为每个资产每天的分组.
            2. dict: 资产-分组映射的字典. 如果传递了dict，则假定分组映射在整个时间段内
            保持不变.
            3. function: 输入参数为 securities, start_date, end_date, 返回值为分组数
            据的 DataFrame 或 dict.
        weights : 权重数据, 默认为 1
            # pd.DataFrame / dict / function
            1. pd.DataFrame: index 为日期, columns 为资产，为每个资产每天的权重.
            2. dict: 资产-权重映射的字典. 如果传递了dict，则假定权重映射在整个时间段内
            保持不变.
            3. function: 输入参数为 securities, start_date, end_date返回值为权重数据
            的 DataFrame 或 dict
        binning_by_group : 是否分析分组（行业）中性的组合, 默认为 False
            # bool
            如果为 True, 则对每个组分别计算分位数. 适用于因子值范围在各个组上变化很大的
            情况.如果要分析分组(行业)中性的组合, 您最好设置为 True
        quantiles : 用于进行分组的分位数, 默认为 None
            # int / sequence[float]
            1. int: 在因子分组中按照因子值大小平均分组的组数
            2. sequence[float]: 分位数序列, 允许不均匀分组. 例如 [0, .10, .5, .90, 1.]
            或 [.05, .5, .95]
            'quantiles' 和 'bins' 有且只能有一个不为 None
        bins : 分组数量, 默认为 None
            # int or sequence[float]
            1. int: 在因子分组中使用的等宽 (按照因子值) 区间的数量
            2. sequence[float]: 边界值序列, 允许不均匀的区间宽度.例如 
            [-4, -2, -0.5, 0, 10]
            'quantiles' 和 'bins' 有且只能有一个不为 None
        periods : 远期收益的期数, 默认为 (1, 5, 10)
            # int or sequence[int]
        max_loss : 允许的丢弃因子数据的最大百分比 (0.00 到 1.00), 默认为 0.25
            # float
            计算比较输入因子索引中的项目数和输出 DataFrame 索引中的项目数.因子数据本身存
            在缺陷 (例如 NaN),没有提供足够的价格数据来计算所有因子值的远期收益，或者因为
            分组失败, 因此可以部分地丢弃因子数据.设置 max_loss = 0 以停止异常捕获.
        zero_aware : 适用于您的信号聚集并且零是正值和负值的分界线的情况. 默认为 False
            # bool
    ...
    def __gen_clean_factor_and_forward_returns(self):
        ...
        # hasattr(x,'call') --> 用来判断一个对象是否存在某个方法
    ...
    def change_params(self, groupby=None, quantiles=None, bins=None, 
                      periods=None, binning_by_group=False, 
                      max_loss=None, zero_aware=False):
        ...
        if periods is not None:
            self._periods = ensure_tuple(periods)
        if binning_by_group is not None:
            self._binning_by_group = binning_by_group
