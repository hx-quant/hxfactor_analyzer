...
from Analyzer.hxfactor_analyzer.hx_exceptions import MaxLossExceededError, non_unique_bin_edges_error
from Analyzer.hxfactor_analyzer.hx_utils import get_forward_returns_columns
...
def compute_forward_returns(factor,
    ...
    """
    功能：创建DataFrame用于存放结果
    # pd.MultiIndex.from_product([],names=[]) --> 用于创建复合索引
    """
    ...
    # pct_change(period).shift(-period)  --> 数据前移，这样就得到当前日期的远期收益率
    # stack() --> 将列名放到索引中，使得df变成了二重索引
    """
...
         或分位数序列, 允许不均匀分组
    ...
    initial_amount = float(len(factor.dropna().index))
